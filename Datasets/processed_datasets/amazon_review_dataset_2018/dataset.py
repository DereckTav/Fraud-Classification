...
class classifyDataset(Dataset):
    def __init__(self, reviews, ratings, types, tokenizer, max_length=512, include_text=False):
    ...
    def __getitem__(self, item):
        product_type = str(self.types[item])
...
def custom_collate_fn(batch, tokenizer):
    ...
    output["product_type"] = [item["product_type"] for item in batch]
...
def get_dataloader(df, tokenizer, batch_size=16, max_length=512, include_text=False, num_workers=0):
    ds = classifyDataset(
        ...
        types=df['product_type'].to_numpy(), 
        tokenizer=tokenizer,

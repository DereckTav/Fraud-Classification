...
from models.model import FraudClassifier
from utils.metrics import plot_training_history
from Datasets.processed_datasets.fraud_data.classification_data import classificationData
...
def main(config_path):
    ...
    os.makedirs('BERT_model/Fraud Classification/models/saved', exist_ok=True)
    os.makedirs('BERT_model/Fraud Classification/outputs/figures', exist_ok=True)
    ...
    fraud_data = classificationData()
    df = fraud_data.get_data()
    ...
    # Initialize model
    model = FraudClassifier(N_CLASSES, MODEL_NAME).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_accuracy:
            model_path = f"BERT_model/Fraud Classification/models/saved/model_state_{timestamp}_no_title_acc{val_acc:.4f}.bin"
            torch.save(model.state_dict(), model_path)
            torch.save(model.state_dict(), 'BERT_model/Fraud Classification/models/saved/best_model_state.bin')
    ...
    history_path = f"BERT_model/Fraud Classification/outputs/logs/training_history_{timestamp}.csv"
    hist_df.to_csv(history_path, index=True, index_label="epoch")
    print(f"Training history saved to {history_path}")
    
    # Plot training history
    figure_path = f"BERT_model/Fraud Classification/outputs/figures/training_history_{timestamp}.png"
    plot_training_history(history, save_path=figure_path)
    print(f"Training plot saved to {figure_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train Fraud classification model')

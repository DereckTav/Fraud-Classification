...
def get_predictions(model, data_loader, device):
    ...
    return all_titles, all_reviews, np.array(all_preds), np.array(all_probs), np.array(all_labels)
...
def main(config_path, model_path):
    ...
    titles, reviews, preds, probs, labels = get_predictions(model, test_loader, device)
    ...
    cm_path = "BERT_model/Sentiment_Analysis/outputs/figures/confusion_matrix.png"
    plot_confusion_matrix(labels, preds, class_names=class_names, save_path=cm_path)
    print(f"Confusion matrix saved to {cm_path}")
    
    # Save predictions to CSV
    predictions_df = pd.DataFrame({
        'title': titles if titles else np.nan,
        'review': reviews if reviews else np.nan,
        'true_label': labels,
    ...
    # Save predictions to CSV
    predictions_path = "BERT_model/Sentiment_Analysis/outputs/predictions/test_predictions.csv"
    ...
    for i in range(sample_size):
        print(f"\nSample {i+1}:")
        if titles:
            print(f"Title: {predictions_df['title'].iloc[i]}")
        if reviews:
            review_text = predictions_df['review'].iloc[i]
            print(f"Review: {review_text[:100]}..." if len(review_text) > 100 else review_text)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'model/title.bin'), help='Path to model weights')

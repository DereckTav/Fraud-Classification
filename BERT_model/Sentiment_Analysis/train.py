...
def main(config_path):
    ...
    # df = df.sample(n=20000, random_state=config['data']['random_state']) # sampling data
    df = df.groupby('polarity').sample(n=8000, random_state=config['data']['random_state'])
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_accuracy:
            model_path = f"BERT_model/Sentiment_Analysis/models/saved/model_state_{timestamp}_acc{val_acc:.4f}.bin"
            torch.save(model.state_dict(), model_path)
            torch.save(model.state_dict(), 'BERT_model/Sentiment_Analysis/models/saved/best_model_state.bin')
